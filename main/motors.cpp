...
void setMotorsValueBySensors() {
    if ((readDistanceRight() > MAX_DISTANCE_SIDE) || (readDistanceLeft() > MAX_DISTANCE_SIDE)) { // The minion in front of a wall or in a conjunction
        ...
        if(readDistanceFront() > MAX_DISTANCE_FRONT/2){
          int cm = FRONT_DELAY_CORRECTION/20;
...
void goBack(){
    if ((readDistanceFront() > MAX_DISTANCE_SIDE) && (readDistanceRight() > MAX_DISTANCE_SIDE) && (readDistanceLeft() > MAX_DISTANCE_SIDE)){
        isGoBack = false;
        isFinish = false;
        isGameOver = true;
        turn_90_degree_right();
    ...
    else if ((readDistanceRight() > MAX_DISTANCE_SIDE) || (readDistanceLeft() > MAX_DISTANCE_SIDE)) { // The minion in front of a wall or in a conjunction
