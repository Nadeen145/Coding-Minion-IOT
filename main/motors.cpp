...
void setMotorsValueBySensors(int distance_front, int distance_right, int distance_left) {
    if ((distance_front < MIN_DISTANCE_RIGHT_SIDE) || (distance_right > MAX_DISTANCE_SIDE) || (distance_left > MAX_DISTANCE_SIDE) { // The minion in front of a wall or in a conjunction
        turn current = instructions.back();
        instructions.pop_back();

        switch (currnt) {
            case(Right_A): color_A(); current = Right; break;
            case(Right_B): color_B(); current = Right; break;
            case(Right_C): color_C(); current = Right; break;
            case(Right_D): color_D(); current = Right; break;
            case(Left_a): color_a(); current = Left; break;
            case(Left_b): color_b(); current = Left; break;
            case(Left_c): color_c(); current = Left; break;
            case(Left_d): color_d(); current = Left; break;
            case(Forward_E): color_E(); current = Forward; break;
            case(Forward_e): color_e(); current = Forward; break;
        }
