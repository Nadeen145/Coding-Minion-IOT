...
void havhavAll() {
	for (int i = 0; i < 8; i++) {
		delay(250);
		analogWrite(PIN_RED_R, HIGH);
		analogWrite(PIN_GREEN_R, HIGH);
		analogWrite(PIN_BLUE_R, HIGH);
		analogWrite(PIN_RED_C, HIGH);
		analogWrite(PIN_GREEN_C, HIGH);
		analogWrite(PIN_BLUE_C, HIGH);
		analogWrite(PIN_RED_L, HIGH);
		analogWrite(PIN_GREEN_L, HIGH);
		analogWrite(PIN_BLUE_L, HIGH);
		delay(250);
		analogWrite(PIN_RED_R, LOW);
		analogWrite(PIN_GREEN_R, LOW);
		analogWrite(PIN_BLUE_R, LOW);
		analogWrite(PIN_RED_C, LOW);
		analogWrite(PIN_GREEN_C, LOW);
		analogWrite(PIN_BLUE_C, LOW);
		analogWrite(PIN_RED_L, LOW);
		analogWrite(PIN_GREEN_L, LOW);
		analogWrite(PIN_BLUE_L, LOW);
	}
}
