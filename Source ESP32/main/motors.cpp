...
void setMotorsValueBySensors(int distance_front, int distance_right, int distance_left) {
    if (((distance_front < MIN_DISTANCE_FRONT))
        || ((distance_front > MAX_DISTANCE_SIDE) && (distance_right > MAX_DISTANCE_SIDE) && (distance_left > MAX_DISTANCE_SIDE))){ // The minion in front of a wall or in a conjunction
...
//    }
    ...
    else if(distance_right > distance_left+25){
        Serial.println("Go Right");
        // turn to the right
        motors->stopA();
        motors->forwardB();
    ...
    else if (distance_left > distance_right+25) { // The minion is too close to the wall from the right side
        Serial.println("Go Left");
        // turn to the left
        motors->forwardA();
        motors->stopB();
    ...
    else if ((distance_front > MAX_DISTANCE_SIDE) && (distance_right > MAX_DISTANCE_SIDE) && (distance_left > MAX_DISTANCE_SIDE)){
        isGoBack = false;
        motors->stop();
    }
    else { // The minion is ok, keep moving forward
