...
void turn_little_right() {
    ...
    delay(FORWARD_DELAY_CORRECTION +RIGHT_DELAY_CORRECTION);
    reset();
    motors->forwardB();      
    ...
    motors->stopA();
    motors->forwardB();
    delay(FORWARD_DELAY_CORRECTION -LEFT_DELAY_CORRECTION);
    motors->forwardA();
}
...
void wallOrConjunction(bool isRegular){  
  ...
  if((isRegular == true)){
      ...
      if (current == Left) {
        if(readDistanceLeft() < MIN_MIN_DISTANCE_SIDE){
        ...
        if(readDistanceRight() < MIN_MIN_DISTANCE_SIDE){
          reverse_instructions.pop_back();
          instructions.clear();
          return;
        }
...
void goBackF() {
    if (((readDistanceFront() > MAX_MAX_DISTANCE_SIDE) && (readDistanceRight() > MAX_MAX_DISTANCE_SIDE) && (readDistanceLeft() > MAX_MAX_DISTANCE_SIDE))){
